  ...
  return (
    <PageShell>
      <section className="relative w-full h-full flex flex-col items-center justify-center px-2 sm:px-4 origin-center scale-[0.89] sm:scale-[0.90] md:scale-[0.88] lg:scale-[0.92] mt-[18vh] sm:mt-[10vh] md:mt-[4vh]">
              ...
              <div className="grid grid-cols-1 md:grid-cols-12 gap-3 md:gap-4 items-stretch h-full min-h-0 relative z-10">
                ...
                <div className="md:col-span-5 flex flex-col justify-between h-full">
                  <form onSubmit={submit} className="space-y-2.5 sm:space-y-3 my-auto">
                    {/* Dynamic Guest Greeting Header */}
                    <div className="relative flex items-center justify-between">
                      <div>
                        <span className="label text-[8px] sm:text-[9px] uppercase tracking-[0.25em] text-gold font-bold">
                          ✦ Royal Blessing ✦
                        </span>
                        <h3 className="script text-2.5xl sm:text-3xl md:text-4xl lg:text-5xl text-maroon font-bold drop-shadow-xs leading-tight">
                          Dear {guestName}
                        </h3>
                      </div>
                      <AnimatedLotus className="relative right-0 top-0 w-7 h-7 sm:w-8 sm:h-8" />
                    </div>

                    <p className="display italic text-[11px] sm:text-xs md:text-sm text-maroon-deep/80 leading-relaxed font-medium">
                      Shower your heartfelt love & prayers for the couple's new beginning.
                    </p>
                    ...
                    <div className="flex flex-wrap gap-1.5 pt-0.5">
                      {QUICK_BLESSINGS.map((qb, idx) => (
                        <motion.button
                          ...
                          whileTap={{ scale: 0.95 }}
                          className="text-[9px] sm:text-[10px] bg-gradient-to-r from-gold/15 via-amber-200/30 to-gold/15 hover:from-gold/30 hover:to-gold/30 text-maroon-deep border border-gold/50 rounded-full px-2.5 py-1 transition-all cursor-pointer font-bold shadow-2xs"
                        >
                          {qb}
                        </motion.button>
                      ))}
                    ...
                    <div className="relative">
                      <textarea
                        ...
                        rows={3}
                        maxLength={400}
                        className="w-full bg-cream/90 border-2 border-gold/40 rounded-xl p-2.5 sm:p-3 text-xs sm:text-sm text-maroon-deep placeholder:text-maroon/40 focus:outline-none focus:border-gold focus:ring-2 focus:ring-gold/20 shadow-inner resize-none transition-all leading-relaxed"
                      />
                      <span className="absolute bottom-2 right-2.5 text-[9px] text-gold/70 label font-semibold">
                        {msg.length}/400
                      </span>
                    </div>

                    {err && (
                      <p className="text-destructive text-xs italic font-semibold">
                        {err}
                      </p>
                    )}
        ...
        {/* Mobile Pop-up Modal for Blessings List (ENHANCED ROYAL SCROLL) */}
        <AnimatePresence>
          {showMobileModal && (
            <motion.div
              initial={{ opacity: 0 }}
              ...
              className="fixed inset-0 z-[300] bg-maroon-deep/80 backdrop-blur-md flex items-center justify-center p-3.5 pt-10 pb-24 md:hidden"
            >
              <motion.div
                ...
                className="relative w-full max-w-lg bg-gradient-to-br from-[#FFFDF8] via-[#FDF6E8] to-[#F8EBD4] rounded-2xl sm:rounded-3xl p-4 sm:p-5 shadow-[0_25px_60px_-10px_rgba(122,31,43,0.5),0_0_0_2px_rgba(212,175,55,0.6)] border-2 border-gold/70 max-h-[85vh] flex flex-col overflow-hidden"
              >
                {/* Shimmer Light Beam */}
                <div className="absolute inset-0 bg-gradient-to-r from-transparent via-gold/20 to-transparent -translate-x-full animate-[shimmer_5s_infinite] pointer-events-none" />

                {/* Gold Jali Corner Accents */}
                <JaliCorner className="top-2.5 left-2.5" />
                <JaliCorner className="top-2.5 right-2.5 rotate-90" />
                <JaliCorner className="bottom-2.5 left-2.5 -rotate-90" />
                <JaliCorner className="bottom-2.5 right-2.5 rotate-180" />
                ...
                {/* Modal Header */}
                <div className="flex items-center justify-between border-b-2 border-gold/30 pb-3 mb-3 shrink-0 relative z-10">
                  <div>
                    <span className="label text-[8px] sm:text-[9px] uppercase tracking-[0.25em] text-gold font-bold">
                      ✦ Sacred Wishes ✦
                    </span>
                    <h3 className="script text-2xl sm:text-3xl text-maroon font-bold drop-shadow-xs">
                  ...
                  <motion.button
                    ...
                    whileTap={{ scale: 0.9 }}
                    className="w-8 h-8 rounded-full bg-maroon text-gold border border-gold/60 flex items-center justify-center font-bold text-xs shadow-sm cursor-pointer"
                  >
                    ✕
                  </motion.button>
                </div>
